	...
	{#if testResult}
		...
		>
			<div class="flex items-start gap-3">
				...
				<button
					class="ml-auto {testResult.success ? 'text-green-500 hover:text-green-700' : 'text-red-500 hover:text-red-700'}"
					...
					<div class="flex items-start justify-between gap-4">
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								<h4 class="font-medium text-secondary-900 truncate">{webhook.name}</h4>
								<span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {getStatusColor(webhook.status)}">
									{webhook.status}
								</span>
							</div>
							<p class="mt-1 text-sm text-secondary-500 truncate font-mono">{webhook.url}</p>
							<div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-secondary-400">
