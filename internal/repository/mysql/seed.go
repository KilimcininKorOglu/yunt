...
// Seeder handles database seeding for MySQL.
type Seeder struct {
	repo *Repository
}
...
func (s *Seeder) SeedFromFile(_ context.Context, _ string) error {
	...
	return fmt.Errorf("SeedFromFile is not implemented for MySQL; use SeedFromReader instead")
}
...
func (s *Seeder) createAdminUser(ctx context.Context, config *SeedConfig) (*domain.User, error) {
	// Hash the password
	passwordHash, err := hashPasswordForSeed(config.AdminPassword)
	if err != nil {
...
// hashPasswordForSeed hashes a plaintext password using bcrypt.
...
func (s *Seeder) CreateUserWithMailbox(ctx context.Context, username, email, password string) (*domain.User, *domain.Mailbox, error) {
	// Hash the password
	passwordHash, err := hashPasswordForSeed(password)
	if err != nil {
