...
// UserRepository implements the repository.UserRepository interface for MySQL.
...
func (u *UserRepository) Exists(ctx context.Context, id domain.ID) (bool, error) {
	query := `SELECT COUNT(*) FROM users WHERE id = ? AND deleted_at IS NULL`

	var count int
	if err := u.repo.db().GetContext(ctx, &count, query, string(id)); err != nil {
...
func (u *UserRepository) ExistsByUsername(ctx context.Context, username string) (bool, error) {
	...
	var count int
	if err := u.repo.db().GetContext(ctx, &count, query, username); err != nil {
...
func (u *UserRepository) ExistsByEmail(ctx context.Context, email string) (bool, error) {
	...
	var count int
	if err := u.repo.db().GetContext(ctx, &count, query, strings.ToLower(email)); err != nil {
