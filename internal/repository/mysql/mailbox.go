...
// MailboxRepository implements the repository.MailboxRepository interface for MySQL.
...
// NewMailboxRepository creates a new MySQL mailbox repository.
...
func (m *MailboxRepository) buildListQuery(filter *repository.MailboxFilter, opts *repository.ListOptions, countOnly bool) (string, []interface{}) {
	...
	if filter != nil {
		...
		if filter.Address != "" {
			sb.WriteString(" AND address = ?")
			args = append(args, filter.Address)
		}

		if filter.AddressContains != "" {
			sb.WriteString(" AND address LIKE ?")
...
func (m *MailboxRepository) Exists(ctx context.Context, id domain.ID) (bool, error) {
	query := `SELECT COUNT(*) FROM mailboxes WHERE id = ?`
...
func (m *MailboxRepository) ExistsByAddress(ctx context.Context, address string) (bool, error) {
	query := `SELECT COUNT(*) FROM mailboxes WHERE address = ?`

	var count int
	if err := m.repo.db().GetContext(ctx, &count, query, strings.ToLower(address)); err != nil {
...
func (m *MailboxRepository) DecrementMessageCount(ctx context.Context, id domain.ID, size int64, wasUnread bool) error {
	...
	if wasUnread {
		query = `UPDATE mailboxes SET message_count = GREATEST(0, message_count - 1), 
			unread_count = GREATEST(0, unread_count - 1), total_size = GREATEST(0, total_size - ?), updated_at = ? WHERE id = ?`
	} else {
		query = `UPDATE mailboxes SET message_count = GREATEST(0, message_count - 1), 
			total_size = GREATEST(0, total_size - ?), updated_at = ? WHERE id = ?`
	}
...
func (m *MailboxRepository) UpdateUnreadCount(ctx context.Context, id domain.ID, delta int) error {
	...
	if delta >= 0 {
		...
		query = `UPDATE mailboxes SET unread_count = GREATEST(0, unread_count + ?), updated_at = ? WHERE id = ?`
	}
...
func (m *MailboxRepository) GetDomains(ctx context.Context) ([]string, error) {
	query := `SELECT DISTINCT SUBSTRING_INDEX(address, '@', -1) as domain FROM mailboxes ORDER BY domain`
