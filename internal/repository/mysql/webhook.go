...
// WebhookRepository implements the repository.WebhookRepository interface for MySQL.
...
// NewWebhookRepository creates a new MySQL webhook repository.
...
func (w *WebhookRepository) buildListQuery(filter *repository.WebhookFilter, opts *repository.ListOptions, countOnly bool) (string, []interface{}) {
	...
	if filter != nil {
		...
		if filter.Event != nil {
			sb.WriteString(" AND JSON_CONTAINS(events, ?)")
			eventJSON, _ := json.Marshal(string(*filter.Event))
		...
		if len(filter.Events) > 0 {
			...
			for i, event := range filter.Events {
				conditions[i] = "JSON_CONTAINS(events, ?)"
...
func (w *WebhookRepository) Exists(ctx context.Context, id domain.ID) (bool, error) {
	query := `SELECT COUNT(*) FROM webhooks WHERE id = ?`

	var count int
	if err := w.repo.db().GetContext(ctx, &count, query, string(id)); err != nil {
...
func (w *WebhookRepository) ExistsByURL(ctx context.Context, userID domain.ID, url string) (bool, error) {
	query := `SELECT COUNT(*) FROM webhooks WHERE user_id = ? AND url = ?`

	var count int
	if err := w.repo.db().GetContext(ctx, &count, query, string(userID), url); err != nil {
