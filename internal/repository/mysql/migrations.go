...
// Migrator handles database migrations for MySQL.
...
func (m *Migrator) ensureMigrationsTable(ctx context.Context) error {
	...
	// MySQL 5.7 does not support CREATE INDEX IF NOT EXISTS, so we need to handle the error
	_, _ = m.pool.ExecContext(ctx, indexQuery)
...
func (m *Migrator) applyMigration(ctx context.Context, migration Migration) error {
	...
	// Execute the up SQL
	if migration.UpSQL != "" {
		statements := splitMigrationStatements(migration.UpSQL)
...
func (m *Migrator) rollbackMigration(ctx context.Context, migration Migration) error {
	...
	if migration.DownSQL != "" {
		statements := splitMigrationStatements(migration.DownSQL)
		for _, stmt := range statements {
			if strings.TrimSpace(stmt) == "" {
				continue
			}
...
// splitMigrationStatements splits a SQL string into individual statements.
// It handles semicolons within strings and comments properly.
// This is tailored for MySQL syntax.
func splitMigrationStatements(sqlContent string) []string {
	...
	for i := 0; i < len(runes); i++ {
		...
		if !inString && !inLineComment && prevChar == '/' && char == '*' {
			inBlockComment = true
			current.WriteRune(char)
			prevChar = char
			continue
		}
		if inBlockComment && prevChar == '*' && char == '/' {
			inBlockComment = false
			current.WriteRune(char)
			prevChar = char
			continue
		...
		if !inString && !inBlockComment && char == '-' && i+1 < len(runes) && runes[i+1] == '-' {
			inLineComment = true
		}
