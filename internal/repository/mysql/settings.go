...
// SettingsRepository implements the repository.SettingsRepository interface for MySQL.
...
func (s *SettingsRepository) Save(ctx context.Context, settings *domain.Settings) error {
	...
	query := `INSERT INTO settings (id, data, updated_at) VALUES (?, ?, ?) 
...
func (s *SettingsRepository) Exists(ctx context.Context) (bool, error) {
	query := `SELECT COUNT(*) FROM settings WHERE id = ?`

	var count int
	if err := s.repo.db().GetContext(ctx, &count, query, DefaultSettingsID); err != nil {
