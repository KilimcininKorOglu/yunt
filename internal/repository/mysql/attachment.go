...
// NewAttachmentRepository creates a new MySQL attachment repository.
...
func (a *AttachmentRepository) GetByContentID(ctx context.Context, contentID string) (*domain.Attachment, error) {
	query := `SELECT id, message_id, filename, content_type, size, content_id, 
		disposition, storage_path, checksum, is_inline, created_at 
		FROM attachments WHERE content_id = ?`
...
func (a *AttachmentRepository) Update(ctx context.Context, attachment *domain.Attachment) error {
	...
	query := `UPDATE attachments SET message_id = ?, filename = ?, content_type = ?, 
		size = ?, content_id = ?, disposition = ?, storage_path = ?, checksum = ?, 
		is_inline = ? WHERE id = ?`
...
func (a *AttachmentRepository) Delete(ctx context.Context, id domain.ID) error {
	...
	if _, err := a.repo.db().ExecContext(ctx, "DELETE FROM attachment_content WHERE attachment_id = ?", string(id)); err != nil {
		return fmt.Errorf("failed to delete attachment content: %w", err)
	}

	// Delete attachment record
	result, err := a.repo.db().ExecContext(ctx, "DELETE FROM attachments WHERE id = ?", string(id))
	if err != nil {
...
func (a *AttachmentRepository) DeleteByMessage(ctx context.Context, messageID domain.ID) (int64, error) {
	...
	// Delete attachments
	result, err := a.repo.db().ExecContext(ctx, "DELETE FROM attachments WHERE message_id = ?", string(messageID))
	if err != nil {
...
func (a *AttachmentRepository) Exists(ctx context.Context, id domain.ID) (bool, error) {
	query := `SELECT COUNT(*) FROM attachments WHERE id = ?`

	var count int
	if err := a.repo.db().GetContext(ctx, &count, query, string(id)); err != nil {
...
func (a *AttachmentRepository) ExistsByContentID(ctx context.Context, contentID string) (bool, error) {
	query := `SELECT COUNT(*) FROM attachments WHERE content_id = ?`

	var count int
	if err := a.repo.db().GetContext(ctx, &count, query, contentID); err != nil {
...
func (a *AttachmentRepository) GetContentSize(ctx context.Context, id domain.ID) (int64, error) {
	query := `SELECT LENGTH(content) FROM attachment_content WHERE attachment_id = ?`
...
func (a *AttachmentRepository) GetTotalSizeByMessage(ctx context.Context, messageID domain.ID) (int64, error) {
	query := `SELECT COALESCE(SUM(size), 0) FROM attachments WHERE message_id = ?`
...
func (a *AttachmentRepository) GetByChecksum(ctx context.Context, checksum string) ([]*domain.Attachment, error) {
	query := `SELECT id, message_id, filename, content_type, size, content_id, 
		disposition, storage_path, checksum, is_inline, created_at 
		FROM attachments WHERE checksum = ?`
...
func (a *AttachmentRepository) CleanupOrphaned(ctx context.Context) (int64, error) {
	...
	deleteQuery := `DELETE FROM attachments WHERE id IN 
		(SELECT a.id FROM (SELECT a2.id FROM attachments a2 LEFT JOIN messages m ON a2.message_id = m.id WHERE m.id IS NULL) as a)`
...
func (a *AttachmentRepository) GetStorageStats(ctx context.Context) (*repository.AttachmentStorageStats, error) {
	query := `SELECT 
		...
		COALESCE(SUM(CASE WHEN is_inline = 1 THEN size ELSE 0 END), 0) as inline_size,
		SUM(CASE WHEN is_inline = 0 THEN 1 ELSE 0 END) as regular_count,
		COALESCE(SUM(CASE WHEN is_inline = 0 THEN size ELSE 0 END), 0) as regular_size,
