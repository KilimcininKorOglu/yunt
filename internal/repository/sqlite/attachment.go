...
// AttachmentRepository implements the repository.AttachmentRepository interface for SQLite.
...
// NewAttachmentRepository creates a new SQLite attachment repository.
...
func (a *AttachmentRepository) Exists(ctx context.Context, id domain.ID) (bool, error) {
	query := `SELECT EXISTS(SELECT 1 FROM attachments WHERE id = ?)`
...
func (a *AttachmentRepository) ExistsByContentID(ctx context.Context, contentID string) (bool, error) {
	query := `SELECT EXISTS(SELECT 1 FROM attachments WHERE content_id = ?)`
...
func (a *AttachmentRepository) StoreContent(ctx context.Context, id domain.ID, content io.Reader) error {
	...
	query := `INSERT OR REPLACE INTO attachment_content (attachment_id, content) VALUES (?, ?)`
