...
// Seeder handles database seeding for PostgreSQL.
type Seeder struct {
	repo *Repository
}
...
func (s *Seeder) createAdminUser(ctx context.Context, config *SeedConfig) (*domain.User, error) {
	// Hash the password
	passwordHash, err := hashPasswordPg(config.AdminPassword)
	if err != nil {
...
// hashPasswordPg hashes a plaintext password using bcrypt.
func hashPasswordPg(password string) (string, error) {
...
func (s *Seeder) CreateUserWithMailbox(ctx context.Context, username, email, password string) (*domain.User, *domain.Mailbox, error) {
	// Hash the password
	passwordHash, err := hashPasswordPg(password)
	if err != nil {
