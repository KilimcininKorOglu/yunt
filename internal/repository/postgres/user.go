...
func (u *UserRepository) buildListQuery(filter *repository.UserFilter, opts *repository.ListOptions, countOnly bool) (string, []interface{}) {
	...
	if filter != nil {
		...
		if filter.Role != nil {
			sb.WriteString(fmt.Sprintf(" AND role = $%d", argIndex))
		...
		if filter.Username != "" {
			sb.WriteString(fmt.Sprintf(" AND LOWER(username) = LOWER($%d)", argIndex))
			args = append(args, filter.Username)
			argIndex++
		}

		if filter.Email != "" {
			sb.WriteString(fmt.Sprintf(" AND LOWER(email) = LOWER($%d)", argIndex))
			args = append(args, filter.Email)
			argIndex++
		}

		if filter.Search != "" {
			sb.WriteString(fmt.Sprintf(" AND (username ILIKE $%d OR email ILIKE $%d OR display_name ILIKE $%d)", argIndex, argIndex+1, argIndex+2))
...
func (u *UserRepository) Update(ctx context.Context, user *domain.User) error {
	...
	query := `UPDATE users SET username = $1, email = $2, password_hash = $3, display_name = $4,
		role = $5, status = $6, avatar_url = $7, signature = $8, signature_html = $9, last_login_at = $10, updated_at = $11 WHERE id = $12`
...
func (u *UserRepository) SoftDelete(ctx context.Context, id domain.ID) error {
	query := `UPDATE users SET deleted_at = $1, updated_at = $2 WHERE id = $3 AND deleted_at IS NULL`
