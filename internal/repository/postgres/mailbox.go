...
func (m *MailboxRepository) Update(ctx context.Context, mailbox *domain.Mailbox) error {
	...
	query := `UPDATE mailboxes SET user_id = $1, name = $2, address = $3, description = $4, 
		is_catch_all = $5, is_default = $6, message_count = $7, unread_count = $8, 
		total_size = $9, retention_days = $10, updated_at = $11 WHERE id = $12`
...
func (m *MailboxRepository) IncrementMessageCount(ctx context.Context, id domain.ID, size int64) error {
	query := `UPDATE mailboxes SET message_count = message_count + 1, 
		unread_count = unread_count + 1, total_size = total_size + $1, updated_at = $2 WHERE id = $3`
