...
func (m *Migrator) loadMigrations() error {
	...
	for _, entry := range entries {
		...
		upSQL, downSQL := parseMigrationContentPg(string(content))
...
// parseMigrationContentPg parses migration SQL content and extracts up/down sections.
// Format expected:
//
//	-- +migrate Up
//	<up SQL statements>
...
func parseMigrationContentPg(content string) (upSQL, downSQL string) {
...
func (m *Migrator) applyMigration(ctx context.Context, migration Migration) error {
	...
	// Execute the up SQL
	if migration.UpSQL != "" {
		statements := splitStatementsPg(migration.UpSQL)
...
func (m *Migrator) rollbackMigration(ctx context.Context, migration Migration) error {
	...
	if migration.DownSQL != "" {
		statements := splitStatementsPg(migration.DownSQL)
		for _, stmt := range statements {
			if strings.TrimSpace(stmt) == "" {
				continue
			}
...
// splitStatementsPg splits a SQL string into individual statements.
// It handles semicolons within strings, dollar-quoted strings, and comments properly.
// PostgreSQL uses $$ for dollar-quoting which is common in functions/triggers.
func splitStatementsPg(sqlContent string) []string {
	...
	inDollarQuote := false
	dollarTag := ""
	var dollarTagBuilder strings.Builder
	inDollarTagCapture := false

	runes := []rune(sqlContent)
	for i := 0; i < len(runes); i++ {
		char := runes[i]

		// Handle dollar-quoting (e.g., $$, $tag$)
		if !inString && char == '$' && !inDollarQuote {
			// Start capturing potential dollar tag
			inDollarTagCapture = true
			dollarTagBuilder.Reset()
			dollarTagBuilder.WriteRune(char)
			continue
		}

		if inDollarTagCapture {
			if char == '$' {
				dollarTagBuilder.WriteRune(char)
				dollarTag = dollarTagBuilder.String()
				inDollarQuote = true
				inDollarTagCapture = false
				...
				// Not a valid dollar tag, write what we captured and continue normally
				current.WriteString(dollarTagBuilder.String())
				inDollarTagCapture = false
		...
		if inDollarQuote && char == '$' {
			// Look ahead for the closing dollar tag
			tagLen := len(dollarTag)
			if i+tagLen <= len(runes) {
				potentialEnd := string(runes[i : i+tagLen])
	...
	// Handle any remaining content from dollar tag capture
	if inDollarTagCapture {
		current.WriteString(dollarTagBuilder.String())
	}
