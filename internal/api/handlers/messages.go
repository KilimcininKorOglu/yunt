...
func (h *MessageHandler) RegisterRoutes(g *echo.Group) {
	...
	messages.POST("/:id/star", h.Star)
	messages.POST("/:id/unstar", h.Unstar)
	messages.POST("/:id/spam", h.MarkAsSpam)
	messages.POST("/:id/not-spam", h.MarkAsNotSpam)
	messages.POST("/:id/move", h.MoveMessage)
